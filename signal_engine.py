...
_CONFIG_PATH = Path(**file**).parent / “signals_config.yml”
...
for _, pos in open_positions.iterrows():
    ...
    # entry_price が NULL（未確定）のポジションは手じまい判定をスキップ
...
# ★ フィルター2: DBの最終日が前営業日から10営業日以上古い銘柄を除外
#    （上場廃止・長期停止銘柄がDBに残っていても除外できる）
today          = _today_jst()
stale_cutoff   = today - timedelta(days=14)  # 14日以上更新がなければ除外
latest_by_ticker = (
...
for ticker, df_ticker in daily_data[daily_data["ticker"] != "NIY=F"].groupby("ticker"):
    ...
    # JPXマスター除外（上場廃止・整理・監理ポスト）
    if jpx_tickers is not None and ticker not in jpx_tickers:
        continue

    # 最終データが古い銘柄を除外（上場廃止でDBに古データが残っているケース）
